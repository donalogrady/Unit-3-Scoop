...
function createArticle(url, request) {
  ...
  /*let requestArticle = {};
  if (request.body){
    requestArticle = request.body.article;
  }
  */
...
//create Comment function I made starts
...
//create comment function I made ends
...
//update an existing comment starts
...
// update an existing comment ends
...
//deleteComment function starts

function deleteComment(url, request){
  //grab number
  const id = Number(url.split('/').filter(segment => segment)[1]);
  //save comment number to variable
  const savedComment = database.comments[id];
  const response = {};
  ...
  //delete comment ID from authors comment IDs
...
//upvote Comment function starts
function upvoteComment(url, request){
  const id = Number(url.split('/').filter(eldolche => eldolche)[1]);
  const username = request.body && request.body.username;
...
//YAML script to load and save database (so no deletion upon exit)


function loadDatabase(object){
